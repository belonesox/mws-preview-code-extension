...
- Интерфейс редактирования в броузере прост, но сильно ограничен возможностями textarea, или визуального редактора ✍️
    ...
    - и даже с лучшими плагинами редакторами типа CodeMirror/WikEd не получается «настоящего IDE»
...
- `mws.sync.command`: команда публикации, по умолчанию настроена чтобы работать с MWS          
  ...
  - **Важно:** все переменные подставляются как есть, без экранирования. Вы должны сами позаботиться о кавычках в командной строке, например: `mws push "${file}" --summary "${summary}"`.
